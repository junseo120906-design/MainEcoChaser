...
export const onRequestPost = async (context) => {
    try {
        ...
        const { pw, nickname, region } = await context.request.json();
        
        if (!pw || !nickname) {
            return new Response(JSON.stringify({ error: "비밀번호와 닉네임은 필수입니다." }), { status: 400 });
        }

        // SQL 쿼리 준비 (D1의 .prepare를 사용)
        const stmt = context.env.DB.prepare(
            "INSERT INTO users (pw, nickname, region) VALUES (?, ?, ?)"
        );

        // 쿼리 실행
        const result = await stmt.bind(pw, nickname, region || null).run();
